...
export function Component({ children, className = "" }) {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        u_noise_scale: { value: 3.6 },
        u_distortion: { value: prefersReducedMotion ? 0.05 : 0.12 },
        u_turbulence: { value: prefersReducedMotion ? 0.35 : 0.65 },
    ...
    const timeline = prefersReducedMotion
      ...
      : gsap.timeline({ repeat: -1, yoyo: true })
          .to(material.uniforms.u_turbulence, { value: 0.92, duration: 8, ease: "sine.inOut" }, 0)
    ...
    const handleMouseMove = (event) => {
      ...
      const mouseInfluence = Math.sqrt(x * x + y * y) * 0.02;
      material.uniforms.u_turbulence.value += mouseInfluence;
    };
  ...
  return (
    <section className={`turbulent-flow ${className}`}>
      <div ref={mountRef} className="turbulent-flow__canvas" aria-hidden="true" />
      <div className="turbulent-flow__shade" aria-hidden="true" />
      <div className="turbulent-flow__content">{children}</div>
    </section>
  );
}
